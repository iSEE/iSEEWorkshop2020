...
initial <- list(
  ...
  ColumnDataPlot(ColumnSelectionSource = "FeatureAssayPlot1", 
                 ColorBy = "Column selection", ColumnSelectionRestrict = FALSE,
                 YAxis = "log10_total", XAxis = "Column data",
                 XAxisColumnData = "subsets_MT_percent", SelectionBoxOpen = TRUE)
)
...
> <i class="fa fa-wrench fa-2x"></i> **Action:** Close the app and run the following block of code to launch the example application described above.
Close the incomplete lasso selection in the first panel by clicking on the initial waypoint.
...
```
```{r, echo=FALSE, out.width = '100%'}
SCREENSHOT("screenshots/isee-iseeu-modereduceddimhex.png", delay=10)
```
